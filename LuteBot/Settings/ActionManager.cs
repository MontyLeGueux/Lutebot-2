...
namespace LuteBot
{
    public class ActionManager
    {
        ...
        private string consoleCommand = "EQUIPMENTCOMMAND";
        ...
        public void ToggleConsole(bool consoleOpen)
        {
            if (consoleOpen != consoleOn)
            {
                ...
                {
                    Enum.TryParse<Keys>(configManager.GetAction("OpenConsole").Code, out Keys tempKey);
        ...
        {
            Process[] processes = Process.GetProcessesByName("Mordhau-Win64-Shipping");
            Enum.TryParse<Keys>(configManager.GetAction("OpenConsole").Code, out Keys consoleKey);
            if (AutoConsoleModeFromString(configManager.GetProperty("AutoConsoleOpen").Code) == AutoConsoleMode.New) {
                foreach (Process proc in processes)
                {
            ...
            foreach (Process proc in processes)
            {
                Thread.Sleep(int.Parse(configManager.GetProperty("NoteCooldown").Code));
